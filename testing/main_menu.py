...
class MainMenu(Entity):
    def __init__(self, player):
        ...
        start_button = Button(text = "Create Server",color = color.gray, highlight_color = color.light_gray,pressed_color = self.color.tint(-.2), scale_y = 0.1, scale_x = 0.3, y = -0.08, parent = self.start_menu)
        # changes the UI to create a server
        start_button.on_click = print("start button to create server")#Func(multiplayer)


        # input fields for later use currently hard coding the IP to localhost and port 55555
        # self.player.host_ip = InputField(default_value= "IP", limit_content_to = "0123456789.localhost", color = color.black, alpha = 100, y = 0.1, parent = self.host_menu)
        # self.player.host_port = InputField(default_value= "PORT", limit_content_to = "0123456789", color = color.black, alpha = 100, y =0.02, parent = self.host_menu)
        ...
        # join_hosted_server = Button(text="Join Server", color = color)
